...
class Ex3(QtWidgets.QDialog):
    def __init__(self, lst, parent=None):
        ...
        self.labelTask = QtWidgets.QLabel(
            'Write a tag answer for each of the following' +
            ' questions (Yes.. . No.. .)')
    ...
    def on_btnCheck_clicked(self):
        ...
        if answer == self.lst[self.i]['response']:
            self.textEdit.clear()
...
if __name__ == "__main__":
    ...
    with open(
         os.path.join(dirname, '../json/ex3.json'), encoding='utf-8') as f:
        data = json.load(f)
    import sys
